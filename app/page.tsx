              ...
              {programDays.map((day, dIdx) => (
                <div key={dIdx} style={{ background: '#1f2937', padding: '12px', borderRadius: '8px', marginBottom: '12px' }}>
                  ...
                  {day.blocks.map((block: any, bIdx: number) => (
                    <div key={block.id} style={{ background: '#111827', padding: '12px', borderRadius: '10px', marginBottom: '12px', border: '1px solid #374151' }}>
                      
                      {/* Selezione Tipo */}
                      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                        <button type="button" onClick={() => updateBlock(dIdx, bIdx, 'type', 'forza')} style={{ flex: 1, padding: '6px', borderRadius: '6px', border: 'none', fontWeight: 'bold', fontSize: '11px', background: block.type === 'forza' ? '#10b981' : '#1f2937', color: '#fff', cursor: 'pointer' }}>🏋️ FORZA</button>
                        <button type="button" onClick={() => updateBlock(dIdx, bIdx, 'type', 'wod')} style={{ flex: 1, padding: '6px', borderRadius: '6px', border: 'none', fontWeight: 'bold', fontSize: '11px', background: block.type === 'wod' ? '#10b981' : '#1f2937', color: '#fff', cursor: 'pointer' }}>🔄 WOD / CIRCUITO</button>
                      </div>

                      {/* Nome Esercizio in rilievo */}
                      <div style={{ marginBottom: '12px' }}>
                        <label style={{ fontSize: '11px', color: '#10b981', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>NOME ESERCIZIO / CIRCUITO</label>
                      ...
                      {block.type === 'forza' ? (
                        <div>
                          {/* Griglia a caselle per Set e Rep */}
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                            <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                              <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>SET</label>
                              <input type="number" value={block.sets} onChange={(e) => updateBlock(dIdx, bIdx, 'sets', e.target.value)} style={{ width: '100%', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', textAlign: 'center', fontWeight: 'bold' }} />
                            </div>
                            <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                              <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>REP</label>
                              <input type="text" value={block.reps} onChange={(e) => updateBlock(dIdx, bIdx, 'reps', e.target.value)} placeholder="es. 5" style={{ width: '100%', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', textAlign: 'center', fontWeight: 'bold' }} />
                            </div>
                          </div>

                          {/* Griglia a caselle per Carico/RPE e Recupero */}
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                            <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                              <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>CARICO / RPE</label>
                              <input type="text" value={block.load} onChange={(e) => updateBlock(dIdx, bIdx, 'load', e.target.value)} placeholder="es. 80% o RPE 8" style={{ width: '100%', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', textAlign: 'center', fontWeight: 'bold' }} />
                            </div>
                            <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                              <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>RECUPERO</label>
                              <input type="text" value={block.rest} onChange={(e) => updateBlock(dIdx, bIdx, 'rest', e.target.value)} placeholder="es. 2 min" style={{ width: '100%', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', textAlign: 'center', fontWeight: 'bold' }} />
                            </div>
                          </div>

                          {/* Casella per le Note */}
                          <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                            <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>NOTE</label>
                            <textarea value={block.notes} onChange={(e) => updateBlock(dIdx, bIdx, 'notes', e.target.value)} placeholder="Aggiungi note tecniche..." style={{ width: '100%', height: '50px', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', fontSize: '12px' }} />
                          </div>
                        </div>
                      ) : (
                        <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px', border: '1px solid #374151' }}>
                          <label style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>TESTO / DESCRIZIONE CIRCUITO</label>
                          <textarea value={block.wodNotes || ''} onChange={(e) => updateBlock(dIdx, bIdx, 'wodNotes', e.target.value)} placeholder="es. AMRAP 12 minuti..." style={{ width: '100%', height: '90px', padding: '6px', background: '#111827', border: '1px solid #374151', color: '#fff', borderRadius: '4px', boxSizing: 'border-box', fontSize: '12px' }} />
                        </div>
                      )}
                    </div>
                  ))}
                  <button onClick={() => addBlockToDay(dIdx)} style={{ width: '100%', padding: '8px', background: '#374151', border: 'none', color: '#fff', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>+ Aggiungi Esercizio</button>
                </div>
              ))}
                ...
                {prog.days?.map((day: any, idx: number) => (
                  <div key={idx} style={{ background: '#1f2937', padding: '10px', borderRadius: '8px', marginBottom: '8px' }}>
                    ...
                    {day.blocks?.map((blk: any, bIdx: number) => (
                      <div key={bIdx} style={{ background: '#111827', padding: '10px', borderRadius: '8px', marginTop: '8px', border: '1px solid #374151' }}>
                        ...
                        {blk.type === 'forza' ? (
                          <div>
                            ...
                            {blk.notes && (
                              <div style={{ background: '#1f2937', padding: '6px', borderRadius: '6px', marginTop: '6px' }}>
                          ...
                          <div style={{ background: '#1f2937', padding: '8px', borderRadius: '6px' }}>
                            <span style={{ fontSize: '10px', color: '#94a3b8', display: 'block', marginBottom: '2px' }}>CIRCUITO / WOD</span>
