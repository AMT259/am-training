...
function bip(frequenza: number, durata: number) {
  // Provo il canale audio del browser: se è davvero attivo lo uso e mi fermo lì.
  // Se non c'è o è bloccato, ripiego sul file incorporato. Mai tutti e due
  // insieme, altrimenti il suono si sente doppio.
  try {
    if (canaleAudio && canaleAudio.state === 'running') {
    ...
    // sospeso: provo a risvegliarlo per la volta successiva
    if (canaleAudio && canaleAudio.state === 'suspended') canaleAudio.resume();
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 3000 }}>
            <div style={{ background: '#ffffff', color: '#000', borderRadius: '12px', maxWidth: '480px', width: '100%', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '12px 20px 18px 20px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '8px' }}>
                <button type="button" onClick={confermaDuplica} style={{ flex: 1, minWidth: 0, padding: '13px', borderRadius: '999px', border: 'none', background: '#10b981', color: '#fff', fontWeight: 'bold', fontSize: '14px', cursor: 'pointer' }}>'use client';
